...
# DewpointC # 176
data, df_copy  = read_dataset(path = 'austin_weather.csv')
# replacement = df_copy['Dew PointC'].mean()
# print(replacement)
# NewDewPointC = data.replace(regex=[r'[-]'], value=replacement)
...
# perceip, cloudCover, events, windDir = data["Precipitationmm"].values, data["CloudCover"].values, data["WindDirDegrees"].values
perceip, events = data["Precipitationmm"].values, data["Events"].values
...
# replace T with 0.009 value for percipitation
df_copy['Precipitationmm'] = df_copy['Precipitationmm'].str.replace('T','0.009')

# meanPerceip, meanCloudCover, meanWindDir = np.mean(perceip), np.mean(cloudCover), np.mean(windDir)
# meanPerceip = np.mean(df['Precipitationmm'].values)
...
# df_copy.to_csv('cleaned_austin_dataset.csv', sep=',')
# read_csv_custom = pd.read_csv('cleaned_austin_dataset.csv')
